...
import { Calendar, Gauge, Fuel, FileText, Info } from "lucide-react";
...
export default function StockGrid() {
  ...
  return (
    <section id="stock" className="min-h-screen flex flex-col justify-center py-20 bg-slate-50 border-y border-neutral-100 snap-start">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="text-center max-w-2xl mx-auto mb-12 lg:mb-16 flex flex-col items-center">
          ...
          {/* Demo Mode Badge */}
          {demoMode && (
            <div className="mt-4 lg:mt-6 flex items-center space-x-2 bg-accent-gold/10 border border-accent-gold/20 px-4 py-2 rounded-lg text-neutral-800 text-[11px] font-semibold">
              <Info className="w-3.5 h-3.5 text-accent-gold shrink-0" />
              <span>Modo Demostración activo. Se muestran coches de ejemplo.</span>
            </div>
          )}
        </div>

        {/* Catalog Container (Flex overflow on mobile, Grid on desktop) */}
        {loading ? (
          <div className="flex overflow-x-auto snap-x snap-mandatory scrollbar-none gap-6 pb-6 md:grid md:grid-cols-2 lg:grid-cols-3 md:gap-8 md:overflow-x-visible md:pb-0">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="shrink-0 w-[85%] sm:w-[60%] snap-center md:w-auto md:shrink bg-white rounded-xl overflow-hidden shadow-xs border border-neutral-200/60 h-[460px] animate-pulse"
              >
          ...
          <div>
            <div className="flex overflow-x-auto snap-x snap-mandatory scrollbar-none gap-6 pb-6 md:grid md:grid-cols-2 lg:grid-cols-3 md:gap-8 md:overflow-x-visible md:pb-0">
              {vehicles.map((car) => {
                ...
                return (
                  <div
                    key={car.id}
                    className="shrink-0 w-[85%] sm:w-[65%] snap-center md:w-auto md:shrink bg-white rounded-xl overflow-hidden shadow-xs hover:shadow-xl border border-neutral-200/60 hover:border-accent-gold/40 transition-all duration-300 flex flex-col group"
                  >
                    {/* Image Frame */}
                    ...
                    <div className="p-5 sm:p-6 flex-1 flex flex-col justify-between">
                      <div>
                        {/* Brand name */}
                        <span className="font-display text-[10px] sm:text-xs font-semibold text-accent-gold tracking-widest uppercase">
                          {car.marca}
                        </span>
                        {/* Model name */}
                        <h3 className="font-serif text-lg sm:text-xl font-normal text-neutral-900 group-hover:text-neutral-900 transition-colors mt-1 mb-3">
                          {car.modelo}
                        </h3>
                      ...
                      <div className="border-t border-neutral-100 pt-4 flex items-center justify-between">
                        ...
                        {car.informe_url && (
                          <a
                            ...
                            className="cursor-pointer inline-flex items-center space-x-1.5 font-sans text-[10px] font-bold uppercase tracking-wider border border-neutral-200 hover:border-neutral-800 text-neutral-700 hover:text-neutral-900 px-3 py-2 rounded transition-colors group/btn"
                          >
            ...
            {/* Visual swipe hint (only on mobile) */}
            <div className="flex items-center justify-center space-x-1 mt-4 md:hidden">
