...
export default function Process() {
  ...
  return (
    <section id="proceso" className="py-24 bg-slate-50 border-t border-neutral-100">
      <div className="max-w-7xl mx-auto px-6">
              ...
              return (
                <div key={idx} className="flex flex-col items-center text-center group">
                  ...
                  <div className="relative mb-6">
                    ...
                    {/* Number badge */}
                    <div className="absolute -top-2 -right-2 bg-neutral-900 text-white font-sans text-[10px] font-bold w-6 h-6 rounded-full flex items-center justify-center border border-white shadow-xs">
                      {step.num}
                    </div>
                  </div>

                  {/* Title & Desc */}
                  <h3 className="font-serif text-lg font-medium text-neutral-950 mb-2 group-hover:text-accent-gold transition-colors">
                    {step.title}
                  </h3>
                  <p className="font-sans text-sm text-neutral-600 leading-relaxed max-w-[260px]">
                    {step.desc}
                  </p>
                </div>
              );
        ...
        {/* Security Banner */}
        <div className="mt-20 bg-white border border-neutral-200/60 p-6 rounded-xl flex flex-col md:flex-row items-center justify-between shadow-xs max-w-4xl mx-auto">
          <div className="flex items-center space-x-4 mb-4 md:mb-0">
            ...
            <div>
              <h4 className="font-serif text-base font-semibold text-neutral-900">Seguridad Total del Proceso</h4>
              <p className="font-sans text-xs text-neutral-500">Firmamos contrato de mediación mercantil garantizado ante notario si lo deseas.</p>
            </div>
          </div>
          <a
            href="#quiz"
            className="cursor-pointer font-sans text-xs font-bold uppercase tracking-wider text-accent-red hover:text-red-700 bg-red-50 hover:bg-red-100/60 px-6 py-3 rounded-lg transition-colors text-center"
          >
            Quiero saber más
          </a>
        </div>
